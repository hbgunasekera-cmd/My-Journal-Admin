 ...
 */
  ...
  const generateTravelArticle = async (place) => {
    // 1. Direct key resolution: Fallback to localStorage to avoid React state/window race conditions
    const currentApiKey =
      window.ARTICLE_KEY ||
      import.meta.env.VITE_ARTICLE_KEY ||
      localStorage.getItem('ARTICLE_KEY');

    // 2. Strict Guard Clause: Abort early if key is missing or invalid
    if (!currentApiKey || currentApiKey === 'undefined' || currentApiKey.trim() === '') {
      triggerToast("Missing or invalid API key. Please update your settings.");
 ...
 Return ONLY this JSON structure:
 {
  ...
  "governing_org": "Open" | "Department of Wildlife Conservation" | "Department of Forest Conservation" | "Central Cultural Fund" | "Department of Archaeology" | "Department of National Botanic Gardens" | "National Livestock Development Board" | "Local Authorities"
    ...
    try {
      ...
      if (typeof triggerToast === 'function') {
        triggerToast(`Metadata Error: ${err.message}`);
      }
      return false;
    }
  };
